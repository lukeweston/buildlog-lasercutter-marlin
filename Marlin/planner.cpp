  ...
  else
  {
    ...
    if (laser.mode == RASTER || laser.mode == PULSED) {
	    ...
	    for (int i = 0; i < LASER_MAX_RASTER_LINE; i++) {
			...
			float NewValue;
			OldRange = (255 - 0);
			NewRange = (laser.rasterlaserpower - 7); //7% power on my unit outputs hardly any noticable burn at F3000 on paper, so adjust the raster contrast based off 7 being the lower.
								// 7 still produces burns at slower feed rates, but getting less power than this isn't typically needed at slow feed rates.
